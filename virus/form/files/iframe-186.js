...
var country;
 ...
 function showCity(o){
    ...
    switch (country) {
            ...
            // $(".phone").mask(phone[country],{placeholder:"", autoclear: false});
            $(".contry_money").text(price[country]);
            $(".contry_name").text(contry_name);    
            $(".contry_name2").text(contry_name2);    
            $('.orderForm').attr('onsubmit', "if(this.name.value==''){alert('Введите Ваше имя!');return false}if(this.phone.value.length < "+phoneLength[country]+"){alert('Номер телефона должен содержать "+phoneLength[country]+" цифр! Пример : "+primer[country]+"');return false;}return true;");
    }
